...
q_request_duration = 'max(rate(istio_request_duration_milliseconds_count{pod=~"frontend-.*", request_protocol="grpc", response_code="200"}[1m]))'
# cpu utilisation [%]
q_cpu_usage = 'rate(container_cpu_usage_seconds_total{pod=~"frontend-.*"}[1m])/0.1*100'
# memory usage [%]
q_memory_usage = 'rate(container_memory_usage_bytes{pod=~"frontend-.*"}[1m])/64000000'
