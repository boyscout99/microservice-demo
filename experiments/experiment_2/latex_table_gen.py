...
with open('exp2_samples.json') as f:
    data = json.load(f)
...
columns = ['rep', 'rps', 'p95', 'cpu', 'mem']

# Generate the LaTeX table header
table_header = '\\begin{tabular}{|c|c|c|c|c|}\n'
table_header += '\\hline\n'
table_header += f"Replicas & RPS [req/s] (std {std_means['rps']}, stderr {stderr_means['rps']}) & p95 (std {std_means['p95']}, stderr {stderr_means['p95']}) & cpu (std {std_means['cpu']}, stderr {stderr_means['cpu']}) & mem (std {std_means['mem']}, stderr {stderr_means['mem']}) \\\\\n"
table_header += '\\hline\n'
