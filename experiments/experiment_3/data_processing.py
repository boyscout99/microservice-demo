...
metrics_df_order = ["Replicas", "Load [req/s]", "RPS [req/s]", "Latency [ms]", "CPU [\%]", "Memory [\%]"]
...
for df in metrics_df:
    ...
    mean_signal = []
    # remove rows
    df.drop([61,62,63,64,65,66,67,68,69,
    ...
    for col in df:
        ...
        axs[metric_idx].plot(t, df[col].values, color='b', label=f"{col}")
    # plot the mean signal
    axs[metric_idx].plot(t, mean_signal, color='r')
